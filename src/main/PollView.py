...
class PollView(disnake.ui.View):
    ...
    def init_embed(self) -> disnake.Embed:
        self.embed = MyEmbed(inter=self.inter, title="投票先を選択", description=self.make_embed_description(), color=disnake.Color.magenta())
        for member in self.vc_members:
            button = disnake.ui.Button(label=member.display_name, style=disnake.ButtonStyle.blurple, custom_id=str(member.id))
    ...
    async def start(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            embed = MyEmbed(inter=inter, title="まだ誰も投票していません.", description="投票が終わってから結果を表示してください.", color=disnake.Color.red())
